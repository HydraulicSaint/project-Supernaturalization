...
export default async function AdminPage() {
  const [issues, conflicts, stale, lowConfidence, decisions, activity] = await Promise.all([
    ...
    load<any>("/api/internal/activity/recent?limit=20")
  ]);

  return (
    <main style={{ padding: 24, fontFamily: "sans-serif" }}>
      <h1>Operator Evidence Board</h1>

      <h2>Unreviewed Issues</h2>
      <ul>{issues.data.map((i: any) => <li key={i.id}><a href={`/admin/cases/${i.source_record_id || ""}`}>{i.issue_type}</a> · {i.severity} · {i.created_at}</li>)}</ul>

      <h2>Unreviewed Conflicts</h2>
      <ul>{conflicts.data.map((c: any) => <li key={c.id}><a href={`/admin/cases/${c.case_id}`}>{c.conflict_type}</a> · {c.severity} · {c.created_at}</li>)}</ul>

      <h2>Stale Enrichment</h2>
      <ul>{stale.data.map((s: any) => <li key={s.id}><a href={`/admin/cases/${s.case_id}`}>{s.case_id}</a> · {s.captured_at}</li>)}</ul>
      ...
      <ul>{decisions.data.map((d: any) => <li key={d.id}><a href={`/admin/cases/${d.case_id}`}>{d.decision_type}</a> · {d.rule_triggered}</li>)}</ul>

      <h2>Recent Operator Actions</h2>
      <ul>{activity.actions.data.map((a: any) => <li key={a.id}>{a.action_type} · {a.actor_display_name || a.actor_id} · {a.target_entity_type}:{a.target_entity_id || "-"}</li>)}</ul>
    </main>
  );
}
