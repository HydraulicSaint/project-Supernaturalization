  ...
  return (
    <AdminShell title={displayName} description="Evidence-chain drilldown organized for review, provenance inspection, and queue follow-through.">
      ...
      <MetaList items={[
        ...
        { label: "Counts", value: `${model.summary.sourceCount} sources · ${model.summary.locationCount} locations · ${model.summary.enrichmentCount} enrichment snapshots · ${model.summary.decisionCount} decisions` }
      ]} />
          ...
          {model.conflicts.length ? model.conflicts.map((conflict: any) => (
            <div className="evidence-card" key={conflict.id}>
              ...
              <KeyValueTable rows={[
                ...
                { key: "Normalized values", value: <JsonBlock value={conflict.normalized_competing_values ?? { previous: conflict.normalized_previous_value, next: conflict.normalized_next_value }} /> }
              ]} />
            </div>
        ...
        {model.decisions.length ? model.decisions.map((decision: any) => (
          <div className="evidence-card" key={decision.id}>
            <div className="evidence-card-header">
              ...
              <Badge tone={getConfidenceTone(decision.confidence)}>{decision.confidence !== undefined && decision.confidence !== null ? Number(decision.confidence).toFixed(2) : "—"}</Badge>
            </div>
            <KeyValueTable rows={[
              { key: "Rule triggered", value: decision.rule_triggered ?? "—" },
              { key: "Actor", value: decision.actor_id ?? "system" },
        ...
        {model.actions.length ? model.actions.map((action: any) => (
          <div className="evidence-card" key={action.id}>
            <div className="evidence-card-header">
              ...
              <Badge tone="neutral">{action.actor_id ?? "internal-operator"}</Badge>
            </div>
            <KeyValueTable rows={[
